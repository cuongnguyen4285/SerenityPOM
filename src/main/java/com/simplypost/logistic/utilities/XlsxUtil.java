...
public class XlsxUtil {
     ...
     */
    public List<String[]> readEachRowDataFromXLSFile(String fileName, String sheetName) throws IOException {
    ...
    /**
     * Read all rows of a column in a sheet of excel file
     *
     * @param fileName
     * @param sheetName
     * @param columnName
     * @return list of String[], each line is a String[]
     */
    public String[] readSpecificColumnDataFromXLSFile(String fileName, String sheetName, String columnName) throws IOException {
        BufferedInputStream input = null;
        Workbook wb = null;
            ...
            for (int i = 0; i < columnCount; i++) {
                if (sheet.getRow(0).getCell(i).getStringCellValue().equalsIgnoreCase(columnName)) {
                    for (int j = 0; j < rowCount; j++) {
                        Cell cell = sheet.getRow(j+1).getCell(i, Row.CREATE_NULL_AS_BLANK);
                        switch (cell.getCellType()) {
                            case Cell.CELL_TYPE_BLANK:
                                columnData[j] = "";
                                break;
                            case Cell.CELL_TYPE_BOOLEAN:
                                columnData[j] = String.valueOf(cell.getBooleanCellValue());
                                break;
                            case Cell.CELL_TYPE_NUMERIC:
                                if (DateUtil.isCellDateFormatted(cell)) {
                                    Date d = cell.getDateCellValue();
                                    columnData[j] = new SimpleDateFormat("dd/MM/yyyy").format(d);
                                } else {
                                    columnData[j] = String.valueOf(cell.getNumericCellValue()).replaceFirst("\\.0+$", "");
                                }
                                break;
                            case Cell.CELL_TYPE_STRING:
